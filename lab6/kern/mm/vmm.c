...
/* do_pgfault - Page Fault Handler
 * @mm:         Memory management struct of current process
 * @error_code: Error code (cause of page fault)
 * @addr:       Virtual address that triggered page fault (stored in stval)
 * 
 * This function handles page faults by:
 * 1. Finding the VMA that contains the faulting address
 * 2. Allocating a new page and mapping it to the address
 * 
...
int do_pgfault(struct mm_struct *mm, uint32_t error_code, uintptr_t addr)
{
    ...
    // If page table entry is not valid, allocate a new page
    if (*ptep == 0) {
        if (pgdir_alloc_page(mm->pgdir, la, perm) == NULL) {
            return -E_NO_MEM;
        }
    } else {
        // Page exists but not accessible, might be permission issue
