...
### 提问 1：如何证明 STRIDE_MAX – STRIDE_MIN <= PASS_MAX？

**证明（使用数学归纳法）：**

**基础情况：** 
初始时，所有进程的 stride 值都为 0，因此：
$$STRIDE\_MAX - STRIDE\_MIN = 0 \leq PASS\_MAX$$

**归纳假设：**
假设在第 $k$ 次调度前，有 $STRIDE\_MAX - STRIDE\_MIN \leq PASS\_MAX$ 成立。

**归纳步骤：**
在第 $k$ 次调度时，调度器选择**当前 stride 最小**的进程执行（设为进程 P）。

执行后，进程 P 的 stride 更新为：
$$stride_P' = stride_P + pass_P$$

其中 $stride_P = STRIDE\_MIN$（因为 P 是被选中的最小 stride 进程）。

更新后的最大 stride 差值分析：

1. **新的 STRIDE_MIN'**：由于原来的最小值进程 P 增加了 $pass_P$，新的最小值 $STRIDE\_MIN' \geq STRIDE\_MIN$（可能是 P 更新后的值，也可能是其他进程）

2. **新的 STRIDE_MAX'**：
   - 如果 $stride_P + pass_P > STRIDE\_MAX$，则 $STRIDE\_MAX' = stride_P + pass_P$
   - 否则 $STRIDE\_MAX' = STRIDE\_MAX$

**情况一**：$STRIDE\_MAX' = STRIDE\_MAX$（原最大值不变）
$$STRIDE\_MAX' - STRIDE\_MIN' \leq STRIDE\_MAX - STRIDE\_MIN \leq PASS\_MAX$$

**情况二**：$STRIDE\_MAX' = stride_P + pass_P$（P 成为新的最大值）

此时新的最小值 $STRIDE\_MIN'$ 必然来自其他某个进程 Q，且 $stride_Q \geq STRIDE\_MIN = stride_P$

$$STRIDE\_MAX' - STRIDE\_MIN' = (stride_P + pass_P) - stride_Q$$
$$\leq (stride_P + pass_P) - stride_P = pass_P \leq PASS\_MAX$$


### 提问 2：BigStride 应该取多少？

**分析：**

根据上述结论，我们有：
$$STRIDE\_MAX - STRIDE\_MIN \leq PASS\_MAX = BIG\_STRIDE$$

为了利用**有符号整数比较**来正确判断两个无符号 stride 值的大小关系（即使发生溢出），我们需要保证：

当计算 $stride_A - stride_B$ 时，结果用**有符号整数**解释后能正确反映实际大小关系。

**关键洞察：**
- 对于 32 位无符号整数，范围是 $[0, 2^{32}-1]$
- 对于 32 位有符号整数，范围是 $[-2^{31}, 2^{31}-1]$

当两个 stride 之差的绝对值不超过 $2^{31}-1$ 时，将差值解释为有符号整数可以正确判断大小：
- 如果真实差值为正且 $\leq 2^{31}-1$，有符号解释也为正
- 如果真实差值为负且 $\geq -(2^{31}-1)$，有符号解释也为负

**结论：**

$$BIG\_STRIDE \leq 2^{31} - 1 = 0x7FFFFFFF$$
...
static int proc_stride_comp_f(void *a, void *b) {
    struct proc_struct *p = a;
    struct proc_struct *q = b;
    int32_t c = p->lab6_stride - q->lab6_stride;  // 有符号比较
    if (c > 0) return 1;
...
**数值验证（以题目中的 16 位为例）：**
- $BIG\_STRIDE_{16bit} = 2^{15} - 1 = 32767$
- 当 $stride_A = 98$，$stride_B = 65535$ 时
- $98 - 65535 = -65437$（数学上）
- 但作为 16 位无符号运算：$98 - 65535 \equiv 99 \pmod{65536}$
- 将 99 解释为有符号 16 位整数仍为 99（正数），说明 $98 > 65535$（在 stride 语义下）

这与理论分析一致：$98 - 65535$ 的真实差值应该是 $98 - 65535 = -65437$，但由于溢出前 A 实际上领先 B（stride 更大），所以比较结果应该是 A > B。
